...
parameters {
  ...
  real<lower=0> mu_sticky;
  real<lower=0> sigma_sticky;
  ...
  //vector[nSub] dA_pr;
  //vector[nSub] dB_pr;
  vector[nSub] sticky_pr; // sticky bonus
}
...
transformed parameters{
  ...
  vector<lower=0,upper=20>[nSub] sticky;
  ...
  for (s in 1:nSub){
    ...
    sticky[s] = Phi_approx(mu_sticky + sigma_sticky * sticky_pr[s])*20;
  }
}
...
model {
 ...
 // mu_dA ~ normal(0,1);
  ...
  //mu_dB ~ normal(0,1);
  //sigma_dB ~ normal(0,1);
  mu_sticky ~ normal(0,1);
  sigma_sticky ~ normal(0,1);
  ...
  for (s in 1:nSub){
    //Define variables
      ...
      //For sticky bonus
      vector[nC] prev_choice; //keep track of prev choice 
      vector[nC] stickyb; //sticky bonus
      ...
      for (rd in 1:nRd){
        ...
        for (t in 1:nT){
          if (choice[s,rd,t] != 0){
            ...
            //stickiness
            stickyb[c] = prev_choice[c] * sticky[s];
            
            Qstim[c] =  Rmu[s,rd,t,c] + theta[s]*Smu[c] + omega[s]*Ssig[c] + stickyb[c];
            }
            ...
            //update
            //sensory learning 
            if (stim[s,rd,t] == 1) A[choice[s,rd,t]] += 1;
            else B[choice[s,rd,t]] += 1;
